...
換成本系列的話,這一章你在**開規格**——只是規格不再是寫給生成器的一段話,而是寫進資料模型本身(null=留給 AI、source 記誰填的、一字不改)。下一章 AI 補格就是拿這份規格去**下發包**、再逐條**做驗收** AI 有沒有照約定走。甲方思維到這裡升級了:從「寫一段好 prompt」變成「設計一個 AI 改不壞的結構」。
