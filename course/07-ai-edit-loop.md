...
用本系列的話:改背景是**重新發包**(把修改句追加進原規格、連同上一版當參考圖一起送),改文字則是不必發包的自理(即時重繪)。把「貴的發包」和「免費的自理」分清楚,額度才花在刀口上——而下一節「prompt 是可攜帶的資產」更進一步:你**開規格**的產出(那份 prompt 模板)是你帶得走的資產,不綁在自家後端。
